...
%%
%%
ppp = input('Which condition? [Before : 1, After : 3]');
taskN(ppp)
%%
%% stimulus sound
%%
...
winG = tukeywin(N,0.2);
for inum = 1:10
    fname = sprintf('EHtoAE_%02d',inum);
    temp = wavread(fname);
...
if ppp == 1
    ...
    % mnum = mnum(RanRan);
    mnum=[williams(nb),williams(nb),williams(nb)];
    ...
    switch mm
        case 'A'
            if ~pFlag
                ...
                stFlag = 0;
                pause(0.15);
                sound(clickST,44100) %% trigger for stop PTB   
                pause(0.15);
                pFlag = 1;
            end
        ...
        case 'Listen'
            if pFlag
                ...
                %%
                 pause(0.1);
                sound(clickMO,44100) %% trigger for start PTB
                pause(0.1);
                %%
                stNum = toc;
                tttt= 0.0;
